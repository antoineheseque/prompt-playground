...
export class AppModule implements NestModule {
  configure(consumer: MiddlewareConsumer) {
    consumer.apply(LoggerMiddleware).exclude({ path: 'cats', method: RequestMethod.GET }, 'cats/{*splat}').forRoutes(CatsController);
  }
}
```
...
@Injectable()
export class CatsService {
  constructor(private readonly dogsService: DogsService, private readonly configService: ConfigService) {}
...
export class CatsResolver {
  ...
  @Mutation(() => Cat)
  async createCat(@Args('name') name: string, @Args('age', { type: () => Int }) age: number, @Args('breed') breed: string) {
...
async function bootstrap() {
  ...
  const config = new DocumentBuilder().setTitle('Cats API').setDescription('The cats API documentation').setVersion('1.0').addTag('cats').addBearerAuth().build();
